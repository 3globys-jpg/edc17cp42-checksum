...
def _verify_integrity():
    """Звіряє SHA256 запущеного exe з сайдкар-файлом *.sha256.
    Поза збіркою PyInstaller (запуск з джерела) перевірка не виконується."""
    ...
    if not expected or actual != expected:
        ...
        messagebox.showerror(
            "Помилка цілісності",
            "Файл програми було змінено, або файл перевірки цілісності\n"
            f"({os.path.basename(hash_path)}) відсутній.\n\n"
            "Запуск заборонено. Перевстановіть програму з оригінального\n"
            "джерела.")
        root.destroy()
        sys.exit(1)
...
class EDC17Tool(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title(f"EDC17CP42 Tuning Tool v{APP_VERSION} | by {APP_AUTHOR}")
    ...
    def _show_about(self):
        messagebox.showinfo(
            "Про програму",
            f"EDC17CP42 Tuning Tool v{APP_VERSION}\n"
            f"Автор: {APP_AUTHOR}\n\n"
            "Контакти: _____________\n\n"
            "© 2026 3Globys. Усі права захищено.\n"
            "Копіювання, модифікація та розповсюдження програми\n"
